...
public class MyClient implements TokenServiceHttpClient,Observable<TokenChangeObserver<MyToken>>{
	...
	public <T extends AccessToken> T post(String path, AccessTokenGrantRequest payload) {
		...
		logger.info("post_called");
		  ...
		  String secret =  "wY4EtHLqfrRVqKNAW4was3jH0T2SXxXv" + ":" + "iZrH3dJwtohLqQwo";
		 ...
		 Charset charset = Charset.forName("UTF-8");
		  ...
		  Response response = (Response) client.target(UriBuilder.fromUri("http://192.168.243.143:8080/iiq/oauth2/generateToken").build()).request("application/scim+json"). // JSON Request Type
				accept("application/scim+json"). // Response access type - application/scim+json
	...
	@Override
	public List<String> post(String token,String path, String queryMap, String queryName) {
		JSONObject jsonObj = new JSONObject();
		jsonObj.put("queryMap", queryMap);
		jsonObj.put("queryString", queryName);
	            

	
		  Response response = (Response) client.target(UriBuilder.fromUri("http://192.168.243.143:8080/iiq/oauth2"+path).build()).request("application/json"). // JSON Request Type
					accept("application/json"). // Response access type - application/scim+json
					header("Authorization", "Bearer "+token) // Authorization header goes here
					.post(Entity.json(jsonObj.toString())); // body with grant type
		return null;
	}
}
